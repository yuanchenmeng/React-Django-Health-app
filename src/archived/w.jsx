...
const Walking = () => {
  ...
  const ListItem = ({ val, date, distance, steps, unit, editable, onChange}) => {
  ...
  const handleInputChange = (field, value) => {
    const updatedItem = { val, date, distance, steps, unit, editable };
  ...
  if (editable===true){
      ...
      <div className="flex items-center justify-between border-b">
        <input type="text" value={editedDate}
            onChange={(e) => handleInputChange('date', e.target.value)}
            className="w-4/5 bg-amber-100"
        ></input>
        <input type="text" defaultValue={distance}
  ...
  const handleItemChange = useCallback((index, updatedItem) => {
      console.log("doing render outside");
  }, [fid]);
